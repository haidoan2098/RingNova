...
**RefixOS** là dự án xây dựng một OS kernel từ đầu (from scratch) trên nền tảng ARM Cortex-A8, cụ thể là bo mạch BeagleBone Black. Tên gọi xuất phát từ **Reference + Linux** — không phải một bản clone hay port của Linux, mà là một cài đặt độc lập của các cơ chế cốt lõi mà một hệ điều hành hiện đại thực sự cần có.
...
**Thành phần liên quan:** PCB, kernel stack per-process, context switch routine (assembly), kernel allocator (`kmalloc`/`kfree`).
...
| 2 | **System calls** | Interface từ user space vào kernel qua SVC: `write`, `exit`, `yield` |
...
RefixOS được giới hạn có chủ đích. Các thành phần dưới đây nằm ngoài phạm vi:
...
- POSIX compliance
- Signals, IPC
- Bootloader tùy chỉnh

---

## 5. Quan hệ với Linux và GNU

Về tư tưởng thiết kế kernel — high address split, per-process page table, SVC syscall, preemptive scheduling trên ARM — RefixOS giải quyết cùng bài toán theo cùng hướng với Linux. Toolchain sử dụng là GNU (`gcc`, `ld`, `gdb`). Libc được tự viết như một bản thay thế tối giản cho glibc.
...
Khi hoàn thành, RefixOS sẽ khởi động trên BeagleBone Black thật, chạy 2–3 user process cô lập theo cơ chế preemptive scheduling, tiếp nhận system call từ user space, và cung cấp một minimal shell để tương tác — toàn bộ trên một kernel được viết từng dòng từ đầu.
